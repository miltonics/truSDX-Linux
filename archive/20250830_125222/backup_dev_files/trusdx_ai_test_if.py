...
spec = importlib.util.spec_from_file_location("trusdx", os.path.join(parent_dir, "trusdx-txrx-AI.py"))
...
def test_if_command_format():
    ...
    try:
        ...
        if len(response_str) != 38:
            print(f"ERROR: Response should be 38 chars, got {len(response_str)}: '{response_str}'")
            return False
