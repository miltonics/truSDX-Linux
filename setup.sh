...
(
    ...
    sudo apt install -y python3 python3-pip portaudio19-dev pulseaudio-utils 2>&1 | tee -a "$LOG_FILE"
    
    echo "Installing Python packages..." | tee -a "$LOG_FILE"
    pip3 install --user --break-system-packages pyaudio pyserial 2>&1 | tee -a "$LOG_FILE"
...
echo "[6.5/6] Optional INI auto-patch (failsafe) for JS8Call..." | tee -a "$LOG_FILE"
(
    set -eE -o pipefail
...
echo "[6.6/6] Running smoke tests including IF command test..." | tee -a "$LOG_FILE"
(
    set -eE -o pipefail
...
if [[ $PACKAGES_INSTALLED -eq 1 ]]; then
    echo "  ✓ python3, python3-pip, portaudio19-dev, pulseaudio-utils" | tee -a "$LOG_FILE"
...
echo
# Run the new IF command format test
    echo "Running IF command format test..."
    python3 -m trusdx_ai_test_if
    
    if [[ $? -ne 0 ]]; then
        echo "ERROR: IF command format test failed"
        SETUP_SUCCESS=0
    fi
