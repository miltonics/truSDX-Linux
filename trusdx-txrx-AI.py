...
# --- Logger Setup ---
LOG_FILE = "trusdx.log"
LOGGER = logging.getLogger('truSDX')
LOGGER.setLevel(logging.DEBUG)  # Default to debug, command line can override
...
def log(msg, level='info'):
    """Wrapper to redirect old log calls to the new logger."""
    if level == 'debug':
        LOGGER.debug(msg)
    elif level == 'warning':
        LOGGER.warning(msg)
...
def show_persistent_header():
    ...
    LOGGER.info("\033[1;32m" + "═"*80 + "\033[0m")  # Double line for persistence
    LOGGER.info(f"\033[1;36m{get_version_string()}\033[0m - \033[1;33m{BUILD_DATE}\033[0m \033[1;37m[PERSISTENT]\033[0m")
    LOGGER.info("\033[1;37m📡 WSJT-X/JS8Call Connection Settings:\033[0m")
    LOGGER.info(f"\033[1;35m  Radio Model:\033[0m {banner_info['radio_model']} | \033[1;35mCAT Port:\033[0m {PERSISTENT_PORTS['cat_port']}")
    LOGGER.info(f"\033[1;35m  Baud Rate:\033[0m {banner_info['baud_rate']} | \033[1;35mPoll Interval:\033[0m {banner_info['poll_interval']} | \033[1;35mPTT:\033[0m {banner_info['ptt_method']}")
    LOGGER.info(f"\033[1;35m  Audio Device:\033[0m {banner_info['audio_device']} | \033[1;35mSample Rate:\033[0m {banner_info['sample_rate']} | \033[1;35mChannels:\033[0m {banner_info['channels']}")
    LOGGER.info("\033[1;32m" + "═"*80 + "\033[0m")  # Double line for persistence
    LOGGER.info("\033[1;33m⚠️  This banner remains visible during operation for quick reference\033[0m")
    LOGGER.info("")
...
def show_version_info():
    ...
    LOGGER.info("  Rig: Kenwood TS-480")
    LOGGER.info("  Poll Interval: 80ms")
    LOGGER.info(f"  CAT Serial Port: {PERSISTENT_PORTS['cat_port']}")
    LOGGER.info("  Baud Rate: 115200")
    LOGGER.info("  Data Bits: 8")
    LOGGER.info("  Stop Bits: 1")
    LOGGER.info("  Parity: None")
    LOGGER.info("  Handshake: None")
    LOGGER.info("  PTT Method: CAT or RTS/DTR")
    LOGGER.info("\nAudio Configuration:")
    LOGGER.info(f"  Input Device: {PERSISTENT_PORTS['audio_device']}")
    LOGGER.info(f"  Output Device: {PERSISTENT_PORTS['audio_device']}")
    LOGGER.info("  Sample Rate: 48000 Hz")
    ...
    LOGGER.info(f"  ... and {len(TS480_COMMANDS)-10} more commands")
    LOGGER.info("\n" + "="*50)
...
def check_audio_setup():
    ...
    try:
        ...
        if result == '0':
            LOGGER.warning(f"\033[1;33m[AUDIO] Creating TRUSDX audio device...\033[0m")
...
def show_audio_devices():
    ...
    for i in range(pyaudio.PyAudio().get_device_count()):
        LOGGER.debug(pyaudio.PyAudio().get_device_info_by_index(i))
    LOGGER.debug("--- Host APIs ---")
    for i in range(pyaudio.PyAudio().get_host_api_count()):
        LOGGER.debug(pyaudio.PyAudio().get_host_api_info_by_index(i))
...
def receive_serial_audio(ser, cat, pastream):
    try:
        LOGGER.info("receive_serial_audio thread started")
        ...
        while status[2]:
            try:
                ...
                LOGGER.error(f"Serial read error in receive_serial_audio: {e}")
                time.sleep(1) # Avoid busy-looping on error
    except Exception as e:
        LOGGER.critical(f"Unhandled exception in receive_serial_audio: {e}", exc_info=True)
        status[2] = False

def play_receive_audio(pastream):
    try:
        LOGGER.info("play_receive_audio thread started")
        while status[2]:
            try:
                if len(buf) < 2:
                ...
                LOGGER.error(f"PyAudio write error in play_receive_audio: {e}")
                # Potentially reset stream here if errors persist
                time.sleep(0.1)
    except Exception as e:
        LOGGER.critical(f"Unhandled exception in play_receive_audio: {e}", exc_info=True)
        status[2] = False
...
def handle_cat(pastream, ser, cat):
    if(cat.inWaiting()):
        if not status[3]:
            status[3] = True
            LOGGER.info("CAT interface active")
            LOGGER.info("\033[1;32m[CAT] Interface activated\033[0m")
        ...
        try:
            ...
            LOGGER.debug(f"\033[1;36m[DEBUG] Raw CAT data: {raw_data}\033[0m")
            ...
            while b';' in handle_cat.buffer:
                ...
                d = cmd_data + b';'
                LOGGER.info(f"\033[1;35m[CMD] Processing: {d}\033[0m")

                # Try to handle TS-480 command locally first
                ts480_response = handle_ts480_command(d, ser)
                if ts480_response:
                    LOGGER.info(f"\033[1;34m[CAT] \033[0m{d.decode('utf-8', errors='ignore').strip()} \033[1;32m→\033[0m {ts480_response.decode('utf-8', errors='ignore').strip()}")
                    cat.write(ts480_response)
                    cat.flush()
                    LOGGER.debug(f"I: {d}")
                    LOGGER.debug(f"O: {ts480_response} (TS-480 emu)")
                ...
                ser.flush()
                LOGGER.info(f"\033[1;33m[FWD] \033[0m{d.decode('utf-8', errors='ignore').strip()} \033[1;31m→ truSDX\033[0m")

                if d.startswith(b"TX"):
                   status[0] = True
                   LOGGER.info("\033[1;31m[TX] Transmit mode\033[0m")
                   pastream.stop_stream()
                   pastream.start_stream()
                   pastream.read(config['block_size'], exception_on_overflow = False)
                if d.startswith(b"RX"):
                   status[0] = False
                   LOGGER.info("\033[1;32m[RX] Receive mode\033[0m")
                   pastream.stop_stream()
                   pastream.start_stream()

        except Exception as e:
            LOGGER.error(f"CAT error: {e}")
            LOGGER.error(f"\033[1;31m[CAT ERROR] {e}\033[0m")

def transmit_audio_via_serial(pastream, ser, cat):
    try:
        LOGGER.info("transmit_audio_via_serial_cat thread started")
        while status[2]:
            try:
                ...
                LOGGER.error(f"Error in transmit_audio_via_serial loop: {e}")
                time.sleep(1)
    except Exception as e:
        LOGGER.critical(f"Unhandled exception in transmit_audio_via_serial: {e}", exc_info=True)
        status[2] = False

def pty_echo(fd1, fd2):
    try:
        LOGGER.info("pty_echo thread started")
        while status[2]:
            try:
                c1 = fd1.read(1)
                fd2.write(c1)
                ...
                LOGGER.error(f"IOError in pty_echo: {e}")
                break # Exit thread on error
    except Exception as e:
        LOGGER.critical(f"Unhandled exception in pty_echo: {e}", exc_info=True)
        status[2] = False
...
def create_trusdx_sink():
    ...
    if not check_trusdx_sink_exists():
        try:
            LOGGER.info("Creating TRUSDX null-sink...")
            LOGGER.warning(f"\033[1;33m[AUDIO] Creating TRUSDX null-sink...\033[0m")
            ...
            if result.returncode == 0:
                LOGGER.info("TRUSDX sink created successfully")
                LOGGER.info(f"\033[1;32m[AUDIO] TRUSDX sink created successfully\033[0m")
                sink_created_by_script = True
                return True
            else:
                LOGGER.error(f"Failed to create TRUSDX sink: {result.stderr}")
                LOGGER.error(f"\033[1;31m[AUDIO] Failed to create TRUSDX sink: {result.stderr}\033[0m")
                return False
        except Exception as e:
            LOGGER.error(f"Error creating TRUSDX sink: {e}")
            LOGGER.error(f"\033[1;31m[AUDIO] Error creating TRUSDX sink: {e}\033[0m")
            return False
    else:
        LOGGER.info("TRUSDX sink already exists")
        LOGGER.info(f"\033[1;32m[AUDIO] TRUSDX sink already exists\033[0m")
        return True
...
def persist_trusdx_sink():
    ...
    try:
        ...
        if os.path.exists(PULSE_CONFIG_PATH):
            with open(PULSE_CONFIG_PATH, 'r') as f:
                ...
                if 'sink_name=TRUSDX' in content:
                    LOGGER.info("TRUSDX sink already configured for persistence")
                    LOGGER.info(f"\033[1;32m[AUDIO] TRUSDX sink already configured for persistence\033[0m")
                    return True
        ...
        LOGGER.info(f"TRUSDX sink added to {PULSE_CONFIG_PATH} for persistence")
        LOGGER.info(f"\033[1;32m[AUDIO] TRUSDX sink added to {PULSE_CONFIG_PATH} for persistence\033[0m")
        return True

    except Exception as e:
        LOGGER.error(f"Error persisting TRUSDX sink: {e}")
        LOGGER.error(f"\033[1;31m[AUDIO] Error persisting TRUSDX sink: {e}\033[0m")
        return False
...
def cleanup_trusdx_sink():
    ...
    if sink_created_by_script and check_trusdx_sink_exists():
        try:
            LOGGER.info("Removing TRUSDX sink created by script...")
            LOGGER.warning(f"\033[1;33m[AUDIO] Removing TRUSDX sink created by script...\033[0m")
            # Get the module ID
            ...
            if module_id:
                ...
                LOGGER.info(f"TRUSDX sink module {module_id} unloaded")
                LOGGER.info(f"\032[1;32m[AUDIO] TRUSDX sink module {module_id} unloaded\033[0m")
            else:
                LOGGER.warning("Could not find TRUSDX module ID")
                LOGGER.warning(f"\033[1;33m[AUDIO] Could not find TRUSDX module ID\033[0m")

        except Exception as e:
            LOGGER.error(f"Error removing TRUSDX sink: {e}")
            LOGGER.error(f"\033[1;31m[AUDIO] Error removing TRUSDX sink: {e}\033[0m")

def verify_sample_rates():
    """Verify and display sample rate information"""
    LOGGER.info(f"Audio configuration:")
    LOGGER.info(f"  TX sample rate: {config.get('tx_rate', audio_tx_rate)} Hz")
    LOGGER.info(f"  RX sample rate: {config.get('rx_rate', audio_rx_rate)} Hz")
    LOGGER.info(f"  TruSDX internal rate: {audio_tx_rate_trusdx} Hz")

    LOGGER.info(f"\033[1;36m[AUDIO] Audio configuration:\033[0m")
    LOGGER.info(f"\033[1;37m  TX sample rate: {config.get('tx_rate', audio_tx_rate)} Hz\033[0m")
    LOGGER.info(f"\033[1;37m  RX sample rate: {config.get('rx_rate', audio_rx_rate)} Hz\033[0m")
    LOGGER.info(f"\033[1;37m  TruSDX internal rate: {audio_tx_rate_trusdx} Hz\033[0m")
    ...
    if tx_rate != audio_tx_rate:
        LOGGER.warning(f"Warning: TX rate mismatch - using {tx_rate} Hz instead of default {audio_tx_rate} Hz")
        LOGGER.warning(f"\033[1;33m[WARNING] TX rate mismatch - using {tx_rate} Hz instead of default {audio_tx_rate} Hz\033[0m")
    if rx_rate != audio_rx_rate:
        LOGGER.warning(f"Warning: RX rate mismatch - using {rx_rate} Hz instead of default {audio_rx_rate} Hz")
        LOGGER.warning(f"\033[1;33m[WARNING] RX rate mismatch - using {rx_rate} Hz instead of default {audio_rx_rate} Hz\033[0m")

def setup_audio_architecture():
    """Setup complete audio architecture with hardening"""
    if platform not in ["linux", "linux2"]:
        LOGGER.warning("Audio architecture hardening only supported on Linux")
        LOGGER.warning(f"\033[1;33m[AUDIO] Audio architecture hardening only supported on Linux\033[0m")
        return True

    if config.get('direct', False):
        LOGGER.info("Direct audio mode - bypassing virtual sink")
        LOGGER.info(f"\033[1;33m[AUDIO] Direct audio mode - bypassing virtual sink\033[0m")
        return True

    LOGGER.info(f"\033[1;36m[AUDIO] Setting up audio architecture with hardening...\033[0m")
    ...
    if not persist_trusdx_sink():
        LOGGER.warning("Warning: Could not persist TRUSDX sink configuration")
        LOGGER.warning(f"\033[1;33m[WARNING] Could not persist TRUSDX sink configuration\033[0m")
...
def graceful_shutdown():
    ...
    LOGGER.warning(f"\033[1;33m[SHUTDOWN] Performing graceful shutdown...\033[0m")
    cleanup_trusdx_sink()
...
def run():
    try:
        ...
        if not setup_audio_architecture():
            LOGGER.error("Failed to setup audio architecture")
            return
        ...
        if config['verbose']:
            show_audio_devices()
            LOGGER.debug("Audio device = ", find_audio_device(virtual_audio_dev_in), find_audio_device(virtual_audio_dev_out) )
            show_serial_devices()
            LOGGER.debug("Serial device = ", find_serial_device(trusdx_serial_dev) )
            LOGGER.debug("Serial loopback = ", find_serial_device(loopback_serial_dev) )

        if platform == "win32":
            if find_serial_device(loopback_serial_dev):
                LOGGER.warning(f"Conflict on COM port {loopback_serial_dev}: Go to Device Manager, select CH340 device and change in advanced settings COM port other than 8 or 9.")
                time.sleep(1)
            if find_serial_device(cat_serial_dev):
                LOGGER.warning(f"Conflict on COM port {cat_serial_dev}: Go to Device Manager, select CH340 device and change in advanced settings COM port other than 8 or 9.")
                time.sleep(1)
        ...
        if platform != "win32":  # skip for Windows as we have com0com there
           ...
           LOGGER.info(f"Created persistent CAT port: {alt_cat_serial_dev} -> {cat_serial_dev}")
        ...
        try:
            ...
            if platform != "win32":
                ...
                if not os.path.exists(cat_link):
                    LOGGER.warning(f"\033[1;33m[BRIDGE] Creating serial bridge at {cat_link}\033[0m")
                    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "create_serial_bridge.sh")
                    subprocess.run(["/bin/bash", script_path], check=True)
                    LOGGER.info(f"\033[1;32m[BRIDGE] Serial bridge created successfully\033[0m")
                else:
                    LOGGER.info(f"\033[1;32m[BRIDGE] Serial bridge already exists at {cat_link}\033[0m")
            ...
            LOGGER.info(f"\033[1;32m[SERIAL] CAT port configured: {loopback_serial_dev}\033[0m")
        except Exception as e:
            if platform == "win32":
                LOGGER.error("VSPE virtual com port not found: reinstall or enable")
            else:
                LOGGER.error(f"\033[1;31m[ERROR] /dev/pts/x device not found: {e}\033[0m")
        ...
        try:
           ...
           out_stream = pyaudio.PyAudio().open(frames_per_buffer=0, format = pyaudio.paUInt8, channels = 1, rate = audio_rx_rate, output = True, output_device_index = find_audio_device(virtual_audio_dev_in) if virtual_audio_dev_in else -1)
            ...
            if platform == "win32": LOGGER.error("VB-Audio CABLE not found: reinstall or enable")
            else:
                LOGGER.error("port audio device not found: ")
                LOGGER.error("  run in terminal: pactl load-module module-null-sink sink_name=TRUSDX sink_properties=device.description=\"TRUSDX\" && pavucontrol  (hint: sudo modprobe snd-aloop)")
            raise

        try:
            ser = serial.Serial(find_serial_device(trusdx_serial_dev), 115200, write_timeout = 0)
        except Exception as e:
            LOGGER.error("truSDX device not found")
            raise
        ...
        LOGGER.info(f"\033[1;32m[INIT] truSDX-AI Driver v{VERSION} started successfully!\033[0m")
        LOGGER.info(f"\033[1;37m[INFO] Available devices:\033[0m [{virtual_audio_dev_in}, {virtual_audio_dev_out}, {cat_serial_dev}]")
        LOGGER.info(f"\033[1;37m[INFO] Persistent CAT port:\033[0m {alt_cat_serial_dev}")

        # Check and setup audio
        audio_status = check_audio_setup()
        if audio_status:
            LOGGER.info(f"\033[1;32m[AUDIO] TRUSDX audio device ready\033[0m")
        else:
            LOGGER.warning(f"\033[1;33m[AUDIO] TRUSDX audio device needs setup - see instructions\033[0m")

        LOGGER.info(f"\033[1;36m[READY] Waiting for connections from WSJT-X/JS8Call...\033[0m")
        LOGGER.info("")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log-disable', action='store_true', help='Disable logging')
    ...
    # --- Configure Logger based on args ---
    ...
    if config['verbose']:
        LOGGER.info(config)
        LOGGER.info(f"\033[1;36m[AUDIO] Audio architecture hardening enabled (Linux only)\033[0m")
        LOGGER.info(f"\033[1;36m[AUDIO] TRUSDX sink detection and persistence: {'Enabled' if not config['direct'] else 'Bypassed (--direct mode)'}\033[0m")
        LOGGER.info(f"\033[1;36m[AUDIO] Sample rates - TX: {config['tx_rate']} Hz, RX: {config['rx_rate']} Hz\033[0m")
    ...
    try:
        ...
        LOGGER.critical(f"\033[1;31m[ERROR] {e}\033[0m")
        graceful_shutdown()
        time.sleep(3)
